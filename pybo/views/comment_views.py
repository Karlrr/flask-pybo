...
def modify_question(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            return redirect('{}#comment_{}'.format(url_for('question.detail', question_id=comment.question.id),
                                                   comment.id))
    else:
...
def modify_answer(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            Comment.modify_date = datetime.now()
            db.session.commit()
            return redirect('{}#comment_{}'.format(url_for('question.detail', question_id=comment.answer.question.id),
                                                   comment.id))
    else:
        form = CommentForm(obj=comment)
    return render_template('comment/comment_form.html', form=form)
