...
def _list():
    ...
    if so == 'recommend':
        ...
        sub_query = db.session.query(
            Answer.query_id, func.count('*').label('num_answer'))\
            .group_by(Answer.question_id).subquery()
        question_list = Question.query \
            .outerjoin(sub_query, Question.id == sub_query.c.question_id) \
            .oder_by(sub_query.c.num_answer.desc(), Question.create_date.desc())
    else:  # 최근 질문
    ...
    if kw:
        ...
        question_list = question_list \
            ...
            .filter(Question.subject.ilike(search) | Question.content.ilike(search) |        # 질문 내용
